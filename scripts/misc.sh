...
if [[ "$BASE" == "openwrt" ]]; then
    ...
    if [ "$VERSION_MAJOR" -ge 21 ] && [ "$PROFILE" == "rpi-4" ]; then
        ...
        cp packages/luci-app-oled_1.2_all.ipk files/root/luci-app-oled_1.2_all.ipk
        sed -i '/# setup misc settings/ a\rm /root/luci-app-oled_1.2_all.ipk' files/etc/uci-defaults/99-init-settings.sh
        sed -i '/# setup misc settings/ a\opkg install /root/luci-app-oled_1.2_all.ipk --force-downgrade' files/etc/uci-defaults/99-init-settings.sh
    fi
    ...
    # add your custom command for specific target and release branch version here
    ...
    if [ "$TARGET" == "Raspberry Pi 4B" ]; then
        ...
        rm packages/luci-app-oled_1.2_all.ipk
    else
        rm packages/luci-app-oled_1.2_all.ipk
    fi
...
fi  # <--- ini adalah penutup untuk `elif [[ "$BASE" == "immortalwrt" ]]`
